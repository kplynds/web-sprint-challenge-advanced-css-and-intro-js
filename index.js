...
function get20s(data) {
  ...
  for (let i = 0; i < data.length; i++ ) {
    let firstYear = parseInt(data[i].years); //gets year artists were born, working
    let secondYear = parseInt(data[i].years.slice(-4)); //gets year they died, working
        if (firstYear >=1900 && secondYear <= 2000) {
          sArray.push(artists[i].name);
    }
  }
  return sArray;
}
...
let me = {
  ...
  bio: "This dude is the goat. He is truly amazing, talented, the best, and humble."
}
function addArtist (array, newObj) {
  array.push(newObj);
  return array; 
}
